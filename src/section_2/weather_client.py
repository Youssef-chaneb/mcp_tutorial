    ...
    try:
        ...
        try:
            ...
            forecast_response = await client.call_tool(
                "get_weather_forecast", {"city": city, "days": days, "units": units}
            )
            ...
            ), "Missing 'forecast' in forecast response"
            assert (
                len(forecast_result["forecast"]) == days
            ), f"Expected {days} days in forecast"
            ...
            fahrenheit_response = await client.call_tool(
                "get_weather_forecast", {"city": city, "days": 1, "units": "fahrenheit"}
            )

            fahrenheit_result = extract_json_content(fahrenheit_response)
            logger.info(f"Fahrenheit forecast result: {fahrenheit_result}")
            assert (
                fahrenheit_result["forecast"][0]["temperature_unit"] == "°F"
            ), "Temperature unit should be °F"
            ...
            invalid_city_response = await client.call_tool(
                "get_weather_forecast", {"city": "InvalidCity", "days": 1}
            )

            invalid_city_result = extract_json_content(invalid_city_response)
            logger.info(f"Invalid city result: {invalid_city_result}")
            assert (
                "error" in invalid_city_result
            ), "Error message should be returned for invalid city"
            ...
            invalid_city_alerts_response = await client.call_tool(
                "get_weather_alerts", {"city": "InvalidCity"}
            )
            ...
            for test_city in cities:
                ...
                city_forecast_response = await client.call_tool(
                    "get_weather_forecast", {"city": test_city, "days": 1}
                )
                ...
                city_alerts_response = await client.call_tool(
                    "get_weather_alerts", {"city": test_city}
                )
