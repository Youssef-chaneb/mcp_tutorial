...
async def test_server():
    ...
    try:
        ...
        assert (
            add_result["result"] == 50.0
        ), "Addition result didn't match expected output"
        ...
        logger.info(
            f"Sort response content count: {len(sort_response.content) if sort_response.content else 0}"
        )
        ...
        ), f"Sort result didn't match expected output. Got {sort_result}, expected {expected_result}"
        ...
        assert (
            reverse_sort_result == expected_reverse_result
        ), f"Reverse sort didn't match expected output. Got {reverse_sort_result}, expected {expected_reverse_result}"
